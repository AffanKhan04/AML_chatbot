...
\author{\IEEEauthorblockN{Ashar Usmani}
\IEEEauthorblockA{\textit{Computer Science} \\
\textit{FAST NUCES}\\
Karachi, Pakistan \\
k224581@nu.edu.pk}
\and
\IEEEauthorblockN{Affan Hassan}
\IEEEauthorblockA{\textit{Computer Science} \\
\textit{FAST NUCES}\\
Karachi, Pakistan \\
k224445@nu.edu.pk}
\and
\IEEEauthorblockN{Hamza Hussain}
\IEEEauthorblockA{\textit{Computer Science} \\
\textit{FAST NUCES}\\
Karachi, Pakistan \\
k224317@nu.edu.pk}
...
\begin{abstract}
This paper details the design of an agent-driven AI chatbot that is adept at comprehending and deriving meaningful insights from intricate legal documents, more particularly, Anti-Money Laundering (AML) guidelines. Our AI bot is built upon the Retrieval Augmentation Generation (RAG) model that leverages large language models (LLMs) in order to effectively comprehend user intent and then employ a hybrid search approach to extract relevant legal text and finally generate actionable compliance tasks. We have developed custom agents to facilitate this purpose of information retrieval with the support of guard rails that avoid hallucinations.
\end{abstract}
...
Recent advancements in Large Language Models (LLMs) \cite{b1} and Agentic Artificial Intelligence have have enabled new ways for automating the retrieval and comprehension of such complex documentation. Standard LLMs often struggle with legal domains \cite{b2} due to "hallucination" and a lack of grounding in specific organizational documents. By leveraging Retrieval-Augmented Generation (RAG) \cite{b3}, \cite{b4}, AI systems can be grounded in factual domain-specific documentation, minimizing the risk of hallucinations while providing highly specific regulatory advice. Furthermore, Agentic AI extends this capability by employing autonomous agents capable of specific reasoning tasks \cite{b5} such as query intent classification, database lookups, and task synthesis which enables a multi-step execution pipeline rather than a simple text-in, text-out interface.
...
\section{Methodology}
The proposed agentic architecture fundamentally separates the load of a typical RAG system into dedicated specialized agents. The overall methodology/architecture is depicted in Fig. \ref{fig:arch}.
